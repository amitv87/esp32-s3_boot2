...
/* Per-chunk scratch in PSRAM BSS.  Pixels go through here byteswapped from
 * RGB565 native-LE (gui_lite's natural format) into wire BE before DMA —
 * AXS15231B expects high-byte-first per pixel and SPI3 has no halfword
 * byte-swap (BL618's `SPI_DATA_WIDTH_16BIT` + `SPI_BYTE_MSB` doesn't exist
 * on ESP32-S3).  Each blit chunk is bswap16'd into this buffer, the buffer's
 * cache lines are flushed, and DMA runs from here.
 *
 * Aligned to 32 (cache-line) so flushing partial-byte ranges still covers
 * the full leading line — Cache_WriteBack_Addr has an alignment bug that
 * silently drops sub-line ranges. */
static uint16_t s_lcd_swap_scratch[LCD_CHUNK_BYTES / 2] __attribute__((aligned(32)));
...
static void lcd_chunk(const uint8_t *p, uint32_t bytes, bool first)
{
    ...
    /* Byteswap-copy native-LE pixels → wire-BE in scratch, then flush. */
    lcd_bswap_copy(s_lcd_swap_scratch, (const uint16_t *)p, bytes / 2);
    lcd_flush_scratch((uint32_t)s_lcd_swap_scratch, bytes);

    /* IDF-style two-phase under one CS:
     *   1) 4-byte header {0x32, 0x00, RAMWR/RAMWRC, 0x00} single-wire (CPU)
     *   2) pixel data in QUAD via DMA from the byteswapped scratch
    ...
    spi_write_quad_dma(s_lcd_swap_scratch, bytes);
    cs_hi();
}
...
{
    /* `color` is native RGB565 (the same format gui_lite produces — no
     * pre-byteswap by the caller).  We prefill the buffer in native-LE
     * order and let lcd_chunk's byteswap produce wire-BE. */
    uint32_t total = (uint32_t)LCD_WIDTH_NATIVE * LCD_HEIGHT_NATIVE * 2u;
    bool first = true;

    static uint16_t buf[LCD_CHUNK_BYTES / 2] __attribute__((aligned(16)));
    for (uint32_t i = 0; i < LCD_CHUNK_BYTES / 2u; i++) buf[i] = color;

    while (total) {
        uint32_t d = total > LCD_CHUNK_BYTES ? LCD_CHUNK_BYTES : total;
        lcd_chunk((const uint8_t *)buf, d, first);
        first = false;
        total -= d;
    }
}

/* Partial-region blit.  AXS15231B in QSPI mode without the touch-breaking
 * C/D-series init commands silently ignores partial CASET/RASET (the
 * `cr_win_en` gate stays at 0).  So we use full-width CASET, send `(y + h)`
 * full-width rows from row 0, and source rows outside the AABB straight
 * from the surface buffer (their content already matches the panel for
 * those rows, so over-writing is a visual no-op).  The dirty rect lands
 * at its real `(x, y)` because each full-width row carries the surface's
 * actual pixels for that row.  X/w aren't used at the panel level. */
              ...
              uint16_t stride, const uint16_t *buffer)
{
    ...
    /* Caller offset `buffer` to surface[y][x] — back out both to reach
     * surface[0][0] so we read full-width rows. */
    ...
    /* Per-chunk row count: as many rows as scratch + MS_DLEN's 32 KB +
     * descriptor-pool capacity allow. */
    ...
    while (cur_row < total_rows) {
        ...
        /* Copy `rows` full-width rows from the surface into scratch. */
